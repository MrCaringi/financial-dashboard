...
export default function UncategorizedError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("Uncategorized Queue error:", error);
  }, [error]);

  return (
    <div className="min-h-screen flex flex-col gap-6 p-4 pt-12 pb-32">
      <header className="flex items-center gap-3 px-2">
        <Link href="/" className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800/80 flex items-center justify-center text-zinc-300 hover:text-zinc-100 transition-transform active:scale-95">
          <ArrowLeft size={18} />
        </Link>
        <div>
      ...
      <section className="glass-card p-8 rounded-2xl text-center max-w-sm mx-auto space-y-4">
        ...
        <h2 className="text-lg font-bold text-zinc-100">Failed to load transactions</h2>
        <p className="text-xs text-zinc-400 leading-relaxed">
          We encountered an issue retrieving uncategorized imports.
        </p>
        <button
          onClick={reset}
